...
try:
    myappid = 'shekinah.etl.converter.ultimate.3.0'
...
class ExcelToSQLApp:
    def __init__(self, root: ttk.Window):
        ...
        self.sheet_list = [] # Lista de hojas
        self.selected_sheet = ttk.StringVar()
        
        self.db_options = ["🛢️ SQL Server", "🐬 MySQL", "🐘 PostgreSQL", "🪶 SQLite", "🔮 Oracle"]
    ...
    def _init_ui(self) -> None:
        ...
        # --- AQUÍ EMPIEZA EL CAMBIO CLAVE DE ORDEN ---
        # Para evitar que el footer se aplaste, lo definimos y empaquetamos AHORA con side=BOTTOM.
        # Tkinter reserva el espacio del fondo primero, y el resto se llena después.
        
        # 4. FOOTER (Lo movemos antes que el resto para asegurar su espacio)
        # padding=(Left, Top, Right, Bottom) -> Le ponemos 10px abajo para que no toque el borde
        footer_frame = ttk.Frame(main_container, padding=(0, 15, 0, 10)) 
        footer_frame.pack(side=BOTTOM, fill=X) ### CORRECCIÓN: Usar side=BOTTOM

        lbl_status = ttk.Label(footer_frame, textvariable=self.status_msg, font=("Segoe UI", 9, "italic"), bootstyle="secondary")
        lbl_status.pack(side=LEFT)

        # Agregamos 'pady=5' a los botones para que se vean "gorditos" y no pegados
        btn_save = ttk.Button(footer_frame, text="💾 Guardar .SQL", command=self.save_file, bootstyle="primary-outline")
        btn_save.pack(side=RIGHT, padx=5, pady=5) ### CORRECCIÓN: pady agregado
        
        btn_copy = ttk.Button(footer_frame, text="📋 Copiar", command=self.copy_to_clipboard, bootstyle="secondary-outline")
        btn_copy.pack(side=RIGHT, padx=5, pady=5) ### CORRECCIÓN: pady agregado

        # --- AHORA SEGUIMOS CON EL RESTO (TOP -> CENTER) ---

        # 1. ORIGEN DE DATOS
        lbl_frame_top = ttk.Labelframe(main_container, text=" 1. Origen de Datos ", padding=15, bootstyle="primary")
        lbl_frame_top.pack(fill=X, pady=(0, 15), side=TOP) # side=TOP explícito
        
        # Fila 1: Archivo
        row1 = ttk.Frame(lbl_frame_top)
        row1.pack(fill=X, pady=(0, 10))
        btn_browse = ttk.Button(row1, text="📂 Seleccionar Excel", command=self.browse_file, bootstyle="secondary")
        btn_browse.pack(side=LEFT, padx=(0, 10))
        self.entry_file = ttk.Entry(row1, textvariable=self.file_path, state="readonly")
        self.entry_file.pack(side=LEFT, fill=X, expand=YES)
        btn_reset = ttk.Button(row1, text="🔄 Nuevo Proceso", command=self.reset_app, bootstyle="light")
        btn_reset.pack(side=RIGHT, padx=(10, 0))

        # Fila 2: Selección de Hoja
        row2 = ttk.Frame(lbl_frame_top)
        row2.pack(fill=X)
        ttk.Label(row2, text="Seleccionar Hoja:").pack(side=LEFT, padx=(0, 10))
        self.cbo_sheet = ttk.Combobox(row2, textvariable=self.selected_sheet, state="disabled", width=40)
        self.cbo_sheet.pack(side=LEFT)
        ttk.Label(row2, text="(Cargue un archivo para ver las hojas disponibles)", font=("Segoe UI", 8, "italic"), bootstyle="secondary").pack(side=LEFT, padx=10)

        # 2. CONFIGURACIÓN
        lbl_frame_conf = ttk.Labelframe(main_container, text=" 2. Configuración SQL ", padding=15, bootstyle="info")
        lbl_frame_conf.pack(fill=X, pady=(0, 15), side=TOP)
        ...
        # 3. RESULTADOS (Este es el que se expande, pero ahora respetará al footer)
        lbl_frame_res = ttk.Labelframe(main_container, text=" 3. Resultados Generados ", padding=10, bootstyle="secondary")
        lbl_frame_res.pack(fill=BOTH, expand=YES, side=TOP)
    ...
    def confirm_exit_custom(self):
        ...
        if os.path.exists("exit_image.png"):
            try:
                ...
                while self.exit_img_ref.width() > 100: self.exit_img_ref = self.exit_img_ref.subsample(2, 2)
    ...
    def browse_file(self) -> None:
        ...
        if filename:
            ...
            self.status_msg.set("⏳ Leyendo estructura del archivo...")
            self.entry_file.configure(bootstyle="default")
            # Lanzamos hilo solo para leer nombres de hojas (es rapido)
            threading.Thread(target=self._scan_excel_structure, args=(filename,), daemon=True).start()

    def _scan_excel_structure(self, filename: str) -> None:
        try:
            # Usamos ExcelFile para no cargar datos en memoria aun, solo metadatos
            xls = pd.ExcelFile(filename)
            sheets = xls.sheet_names
            
            # Actualizar UI desde hilo principal
            self.root.after(0, lambda: self._update_sheet_combo(sheets, filename))
    ...
    def _update_sheet_combo(self, sheets: list, filename: str):
        """Callback que se ejecuta en el hilo principal tras leer el Excel"""
        self.cbo_sheet['values'] = sheets
        self.cbo_sheet.configure(state="readonly")
        
        if sheets:
            self.cbo_sheet.current(0) # Seleccionar la primera por defecto
            
        clean_name = os.path.splitext(os.path.basename(filename))[0].replace(" ", "_")
        self.table_name.set(clean_name)
        
        self.status_msg.set(f"✅ Archivo detectado. {len(sheets)} hoja(s) encontrada(s).")
        self.is_generated = False

    def start_generation(self) -> None:
        if not self.file_path.get():
            messagebox.showwarning("Validación", "Por favor seleccione un archivo Excel primero.", parent=self.root, icon='warning')
            self.entry_file.configure(bootstyle="danger")
            return

        if not self.selected_sheet.get():
            messagebox.showwarning("Validación", "Por favor seleccione una hoja de cálculo.", parent=self.root, icon='warning')
            return

        self.status_msg.set(f"⚙️ Procesando hoja '{self.selected_sheet.get()}'...")
        self.txt_create.delete(1.0, END)
        self.txt_insert.delete(1.0, END)

        # Ahora lanzamos el hilo pesado de procesamiento de datos
        threading.Thread(target=self._process_data_thread, daemon=True).start()

    def _process_data_thread(self):
        try:
            # AQUI LEEMOS LA DATA REAL DE LA HOJA SELECCIONADA
            ...
            self.root.after(0, lambda: self._show_results(create_sql, insert_sql))
            
        except Exception as e:
            err = str(e)
            self.root.after(0, lambda: self._show_error(err))

    def _show_results(self, create_sql, insert_sql):
    ...
    def _show_error(self, err_msg):
        ...
        messagebox.showerror("Error SQL", f"Fallo al procesar datos:\n{err_msg}", parent=self.root, icon='error')
